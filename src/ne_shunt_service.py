...
class ne_shunt_service:
    ...
    ############################################
    # Occurs when the switch is toggled in the UI
    ############################################
    ...
    def _update(self):
        ...
        for key, value in newData.diff(curData):
            ...
            if key == 'fresh_water_tank':
                ...
                self.update_dbus_item("vehicle_battery", "/Soc",
                                       battery_service.calcVehicleSoc(value))
